...
function Sidebar() {
  ...
  return (
    <div className={`${isOpen ? 'right-0':'-right-full'} w-full bg-white fixed top-0 h-full shadow-2xl md:w-[35vw] xl:max-w-[30vw] transition-all duration-300 z-20 px-4 lg:px-[35px]`} >
      ...
      {/*  */}
      <div className='flex flex-col gap-y-2 h-[27rem] lg:h-[20rem] overflow-y-auto over-x-hidden border-b'>
        {cart.map((item)=>{ 
      ...
      <div className=' flex flex-col gap-y-3 py-4'>
       ...
       <Link to={'/'} onClick={handleClose} className='  bg-primary flex p-4 justify-center items-center text-white w-full font-medium'>Close Cart</Link> 
      </div>
    </div>
  )
}
